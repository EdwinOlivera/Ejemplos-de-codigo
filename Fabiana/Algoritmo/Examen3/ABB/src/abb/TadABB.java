...
//Funcion que cuenta cuantos nodos con 2 hijos

    public int contarNodosHijos() {

        contarNodos(this.Raiz);
        return this.contarNodosGlobales;
    }

    public int contarNodos(TipoNodo Aux) {
        ...
        if (Aux != null) {
            ...
            if (Aux.getIzq() != null && Aux.getDer() != null) {
                this.contarNodosGlobales++;

            }
            contarNodos(Aux.getIzq());
            contarNodos(Aux.getDer());
    ...
    //Esta FUNCION ERA LA QUE COMPROBARIA EL NIVEL DEL ARBOL (SI ESTABA COMPLETO O NO)
    public void ComprobarNivel(int numNivel) {

        TipoNodo Aux = this.Raiz;

        if (Aux != null) {

            TipoInfo tmpDato;
            tmpDato = Aux.getInfo();
            if(Aux.getDer()==null && numNivel==this.NivelActual ){
                System.out.println("El nivel esta incompleto");
            }
            if(Aux.getIzq()!=null){
                this.NivelActual++;
            }
